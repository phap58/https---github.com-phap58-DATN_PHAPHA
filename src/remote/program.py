...
def run_check_internet():
    VM_USER, VM_PASSWORD = s.VM_USER, s.VM_PASSWORD
    VM_USER = '"' + VM_USER + '"'
    VM_PASSWORD = '"' + VM_PASSWORD + '"'
    path_program = "C:\\Users\\admin\\Desktop\\run_check_internet.ps1"
    path_computer = "D:\\May12\\May12.vmx"
    path_computer = '"' + path_computer + '"'
    path_program = '"' + path_program + '"'
    command = s.SCRIPT_CONNECT_TO_SERVER + " " + s.PATH_VMRUN + ' -gu ' + VM_USER + ' -gp ' + VM_PASSWORD + ' runProgramInGuest ' + path_computer + ' -noWait ' + path_program
    try:
